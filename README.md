...
Так как это учебный проект, то я вообще не занимался оптимизацией ни по памяти ни по времени, хотя возможности для этого определенно есть. Например, сейчас при выделении нового блока при отсутсвии свободных ячеек в памяти делается своп самого старого выделенно блока в ram (в соответствии с формальным заданием). При этом, если он залочен, то менеджер просто ждет пока он разлочится. Вместо этого можно было пропускать залоченные блоки и свопать самый старый не залоченный блок. В общем, тут есть над чем еще поработать :)
