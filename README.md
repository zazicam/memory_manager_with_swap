...
Этот недостаток можно исправить переносом блока в другой блок оперативной памяти, но я пока этого не сделал. Однако это возможно и, в общем-то, не сложно сделать в этой реализации, так как мы работаем не с сырыми указателями, а с MemoryBlock и ничто не мешает нам изменять адрес памяти внутри него при необходимости. 

Я не стал этого делать так как в принципе в задании это не оговаривалось и, главное, я понял, что вообще можно сделать все гораздо проще и эффективнее. Очевидно можно реализовать такой же многопоточный менеджер памяти вообще без использования мьютексов и блокировок просто выполняя своп блоков относящихся к тому же потоку, который запрошивает новый блок. При этом, однако, надо использовать 2 уровня ram, чтобы можно было одновременно работать в потоке с любой парой блоков (например, копировать данные из одного блока в другой). Вроде бы очевидное решение, но я, почему-то, додумался до этого только когда текущий вариант c кучей блокировок уже был почти готов... однако, идею посчитал необходимым записать, чтобы не забыть и использовать в будудем. 

## Замечания по производительности <a name="performance-notes"></a>

Во-первых, эта программа не предназначена для копирования файлов, она нужна для тестирования менеджера памяти. Она читает файлы блоками случайных размеров от 0 до 4096 байт, что в случае реального копирования не эффективно. Поэтому по сравнению со стандартными утилитами копирования она работает примерно в 10-20 раз медленнее. У меня папка в 6Гб копировалась 12 минут (в 3 потока), против 1 минуты стандартной системной утилиты.
