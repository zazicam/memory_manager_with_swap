...
Этот недостаток можно исправить переносом блока в другой блок оперативной памяти, но я пока этого не сделал. Однако это возможно и, в общем-то, не сложно сделать, так как мы работаем не с сырыми указателями, а с оберткой MemoryBlock и ничто не мешает нам изменять адрес памяти внутри него при необходимости. 
