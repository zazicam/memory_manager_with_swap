...
Я не стал этого делать так как в задании это не оговаривалось и, главное, я понял, что вообще можно сделать все гораздо проще и эффективнее. Очевидно, можно реализовать такой же многопоточный менеджер памяти вообще без использования мьютексов и блокировок просто выполняя своп блоков относящихся к тому же потоку, который запрaшивает новый блок. При этом, однако, надо использовать 2 уровня ram, чтобы можно было одновременно работать в потоке с любой парой блоков (например, копировать данные из одного блока в другой). Вроде бы, очевидное решение, но я, почему-то, додумался до этого только когда текущий вариант c кучей блокировок уже был почти готов... однако, идею посчитал необходимым записать, чтобы не забыть и использовать в будудем. 
