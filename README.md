...
однако, в других задачах такая необхомость, скорее всего, возникнет. Например, хотелось бы использовать memncpy()
...
Этот недостаток можно исправить переносом блока в другой блок оперативной памяти, но я пока этого не сделал. Однако это возможно и, в общем-то, не сложно сделать, так как мы работаем не с сырыми указателями, а с оберткой MemoryBlock и ничто не мешает нам изменять адрес памяти внутри него при необходимости. Фактически нужно просто сделать проверку блокировка верхнего блока (ram) и, если он залочен, то не ждать пока освободится, а выделить новый блок памяти (getBlock) и переписать туда.  

Я не стал этого делать так как в задании это не оговаривалось и, главное, я понял, что вообще можно сделать эффективнее. Можно реализовать такой же многопоточный менеджер памяти c меньшим количеством блокировок просто выполняя своп блоков относящихся к тому же потоку, который запрaшивает новый блок. При этом, однако, надо использовать 2 уровня ram, чтобы можно было одновременно работать в потоке с любой парой блоков (например, копировать данные из одного блока в другой). При этом блокировки будут нужны только при выделении памяти под новый блок и потоки вообще не будут мешать друг другу в процессе свопа блоков. Вроде бы, очевидное решение, но я, почему-то, додумался до этого только когда текущий вариант c кучей блокировок уже был почти готов... однако, эту идею я считаю важной, поэтому решил записать, чтобы не забыть и использовать в будудем. 

## Замечания по производительности <a name="performance-notes"></a>

Во-первых, эта программа не предназначена для копирования файлов. Она нужна для тестирования менеджера памяти. Она читает файлы блоками случайных размеров от 0 до 4096 байт, что в случае реального копирования не эффективно. Кроме того, я замерял производительность при серьезных ограничениях на использование оперативной памяти, например, при копировании 6Гб данных менеджеру памяти разрешалось исппользовать только 100 Мб оперативки. Поэтому по сравнению со стандартными утилитами копирования она работает примерно в 10-20 раз медленнее. У меня папка в 6Гб копировалась 12 минут (100Мб оперативки, 3 файла - 3 потока), против 1 минуты стандартной системной утилиты.

Во-вторых, я экспериментировал с копированием всех файлов в одном потоке (последовательно) и с копированием каждого файла в отдельном потоке. Как и ожидалось, копирование файлов в одном потоке выполняется быстрее примерно в 2 раза (вероятно из-за отсутсвия блокировок и меньшего количества свопов). Однако при копировании более 10 файлов иногда многопоточная версия выполняла копирование быстрее однопоточной, что удивительно. Я думаю, что это получается просто за счет повышения приоритета процесса с большим числом потоков в ОС Fedora linux, а может и просто случайное стечение обстоятельств. Чтобы можно было с этим поэкспериментировать я оставил в коде обе функции, но закоментировал вызов однопоточной версии.

Так как это учебный проект, то я вообще не занимался оптимизацией ни по памяти ни по времени, хотя возможности для этого определенно есть. Например, сейчас при выделении нового блока при отсутсвии свободных ячеек в памяти делается своп самого старого выделенно блока в ram (в соответствии с формальным заданием). При этом, если он залочен, то менеджер просто ждет пока он разлочится. Вместо этого можно было пропускать залоченные блоки и свопать самый старый не залоченный блок. В общем, возможности для оптимизации определенно есть.
