...
При этом это ограничение действует только на объем памяти напрямую выделяемый менеджером с помощью malloc. Дополнительно в процессе работы менеджер создает разные объекты и списки содержающие служебную информацию о выделенных блоках (адрес, размер и т.п.). В данной реализации на эти данные ограничение не действует. При этом на дополнительно задействуется порядка 40 байт на каждый блок. Это вполне допустимые накладные расходы с учетом того, что в среднем длина блока 512 байт (~8%).
